...
- build: publish an apt, dnf and apk repository at <https://pouriyajamshidi.github.io/packages>, so Debian, Ubuntu, Fedora, RHEL and Alpine users add it once and then `apt install tcping`, `dnf install tcping` or `apk add tcping` like any other package, upgrades included. The metadata and the rpm packages are signed, `tools/build-linux-repo.sh` builds the tree and can be run by hand, and each release replaces only tcping's packages in it, so the same repository can hold other projects later
- build: add an AUR workflow, so `yay -S tcping-bin` picks up a new release without being updated by hand. The package is called `tcping-bin` because it installs the released binary rather than building from source, and because the plain `tcping` name on the AUR belongs to an unrelated project
...
- refactor (breaking): rewrite the JSON output events - each event now carries a minimal, consistent payload (`start`, `probe`, `retry`, `nameResolution`, `statistics`, `error`) instead of one large struct with inconsistent field names; the `info` event type is removed
- feat: every JSON event can carry a `source` field naming the machine that probed, next to its `type`. It is set with `--source-label`, and defaults to the machine's hostname when the events are being sent elsewhere with `--json-url`, so a plain `-j` run does not put the hostname in output that never left the machine
...
- feat: report the standard deviation of the latency (`stddev`) alongside the minimum, average and maximum, the same way `ping` does. It is carried by every printer: the summary line, the JSON `latencyStdDev` field, the CSV `Latency StdDev` row, the sqlite3 `latency_stddev` column, Alloy's `tcping_rtt_milliseconds{stat="stddev"}` and InfluxDB's `rtt_stddev_ms`
...
- feat: add `--source-label` flag to name the machine in the JSON events and in the metrics sent to Alloy and InfluxDB, so that several machines probing the same target do not all write to the same series and a collector can tell their events apart. It defaults to the machine's hostname whenever the results are sent elsewhere, and shows up as the `source` field, label or tag
...
- fix: the container image can probe `https://` targets again. It is built `FROM scratch`, which carries no root certificates, so every HTTPS probe failed the certificate check and only `--insecure` got a reply. The CA bundle is now copied into the image
- docs: `docs/observability` now runs tcping itself. The Compose stack starts nine tcping containers alongside Alloy, Prometheus, InfluxDB and Grafana, covering TCP, HTTP(S) and UDP from two source labels and sending every run to both backends, with one of them acting as the `--udp-server` the UDP probes talk to. The graphs therefore have something in them as soon as the stack is up
- docs: split the Grafana dashboard in `docs/observability` into `tcping (InfluxDB)` and `tcping (Alloy)`, holding the same panels in the same places and linking to each other, instead of one dashboard with the Prometheus copies buried in a trailing row. Flux and PromQL cannot share a panel, so which dashboard to open is simply which way the run was sent. Each dashboard's **Source** dropdown now reads from its own data source, so an Alloy-only run is no longer missing from the list
- docs: make the dashboards readable with several runs on them: the probe result and the UDP reply panels are drawn as state timelines rather than piling stepped lines into one block of colour, the tables no longer cut off their last column, the certificate panel says `83 days` instead of `11.9 weeks`, and the legends name the two ends of a streak, the two probe counters and the four latency stats apart
- docs: both dashboards in `docs/observability` now read their data source from a **Data source** dropdown and the InfluxDB one its bucket from a `bucket` variable, instead of naming the playground's own. Importing either into an existing Grafana is now a paste and a click, with no JSON to edit, and the same file is still what the stack provisions
- docs: add a demo GIF for streaming the JSON output to a collector. It and the UDP one now stack their two panes instead of placing them side by side
...
- docs: split the `docs/observability` Compose stack into one file per way of sending: `compose.prometheus.yml` (OTLP straight to Prometheus, no Alloy or collector needed), `compose.alloy.yml`, `compose.otelcol.yml` and `compose.influxdb.yml`. Each starts on its own, and any of them can be started together with several `-f` flags to compare them. The `tcping (Alloy)` dashboard is now `tcping (Prometheus)`, since all three OTLP paths land there with the same names and labels
...
- refactor (breaking): how long the target had been up, or down, is now part of the probe that ended it, on every printer, instead of a report of its own. The terminal prints `(after 4 seconds of uptime)` on the failure that ends an uptime and `(after 3 seconds of downtime)` on the reply that ends an outage; the JSON `probe` event gained `endedUptime` and `endedDowntime` and lost the `uptimeDuration` and `downtimeDuration` events; CSV gained the `Ended Uptime` and `Ended Downtime` columns and sqlite3 the `ended_uptime` and `ended_downtime` columns; Alloy and InfluxDB send `tcping_last_uptime_seconds`/`tcping_last_downtime_seconds` and `tcping_uptime`/`tcping_downtime` under the same names as before, along with the probe. `probe.Printer` loses `PrintUpTimeDuration` and `PrintDownTimeDuration`, which had come to mean something different in every printer - nothing at all in two of them. The old separate report also repeated a period from an earlier cycle, so a recovery could claim an uptime that had ended minutes ago
- refactor (breaking): `--failures-only` now really means failures only. The probe that ends an outage is a successful one, so it is no longer reported, and neither is the length of the outage
...
- feat: add shell completions for `bash`, `zsh`, `fish` and `PowerShell`, which tcping prints with `--completions <shell>`, e.g. `tcping --completions fish > ~/.config/fish/completions/tcping.fish`. They complete the flags, the interface names for `-I` and the file names for `--csv` and `--sqlite`. The release archives ship them next to the binary and the Debian package installs them
