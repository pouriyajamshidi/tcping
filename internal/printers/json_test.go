...
func jsonEvents(t *testing.T, cfg config.PrinterConfig, f func(p *JSONPrinter)) []map[string]any {
	t.Helper()
...
func printOneJSONEvent(t *testing.T, cfg config.PrinterConfig, f func(p *JSONPrinter)) (string, map[string]any) {
	t.Helper()
...
func TestJSONPrintStart(t *testing.T) {
	t.Run("hostname target reports its resolution time", func(t *testing.T) {
		eventType, data := printOneJSONEvent(t, config.PrinterConfig{}, func(p *JSONPrinter) {
...
func TestJSONSimpleEvents(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			eventType, data := printOneJSONEvent(t, config.PrinterConfig{}, tt.print)
...
func TestJSONPrintsOneEventPerLine(t *testing.T) {
	...
	out := captureStdout(t, func() {
		p := NewJSONPrinter(config.PrinterConfig{})
		p.PrintStart(s)
		p.PrintProbeSuccess(s)
		p.PrintProbeFailure(s)
	})
...
func TestJSONPrettyPrint(t *testing.T) {
	out := captureStdout(t, func() {
		NewJSONPrinter(config.PrinterConfig{PrettyJSON: true}).PrintStart(jsonTestStats())
	})
...
func TestJSONShutdownPrintsStatistics(t *testing.T) {
	eventType, _ := printOneJSONEvent(t, config.PrinterConfig{}, func(p *JSONPrinter) {
...
func TestJSONShutdownOmitsStatistics(t *testing.T) {
	events := jsonEvents(t, config.PrinterConfig{OmitStatistics: true}, func(p *JSONPrinter) {
		p.Shutdown(jsonTestStats())
	})
