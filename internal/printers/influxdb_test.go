...
func influxDBTestConfig(url string) config.PrinterConfig {
...
func TestInfluxDBNeedsOrgBucketAndToken(t *testing.T) {
	tests := []struct {
		name    string
		missing func(cfg *config.PrinterConfig)
	}{
		{"no organization", func(cfg *config.PrinterConfig) { cfg.InfluxDBOrg = "" }},
		{"no bucket", func(cfg *config.PrinterConfig) { cfg.InfluxDBBucket = "" }},
		{"no token", func(cfg *config.PrinterConfig) { cfg.InfluxDBToken = "" }},
	}
