...
func TestNewCSVPrinter(t *testing.T) {
	...
	cp, err := newCSVPrinter(dataFilename, showTimestamp)
...
func TestWriteRecord(t *testing.T) {
	...
	record := []string{"Success", "hostname", "127.0.0.1", "80", "1", "10.123"}
	...
	assert.NoError(t, err)
	assert.Equal(t, []string{"Status", "Hostname", "IP", "Port", "TCP_Conn", "Latency(ms)"}, headers)
